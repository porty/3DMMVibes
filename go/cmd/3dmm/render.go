...
func renderPNGAction(c *cli.Context) error {
	...
	return mm.RenderMovie(c.String("outdir"), c.Int("scene"), c.String("assets"), cf, f, log.New(os.Stderr, "", 0))
}
...
func renderRGB24Action(c *cli.Context) error {
	...
	return mm.RenderMovieRGB24(out, c.Int("scene"), c.String("assets"), cf, f, log.New(os.Stderr, "", 0))
}
...
func renderFFmpegAction(c *cli.Context) error {
	...
	renderErr := mm.RenderMovieRGB24(stdin, c.Int("scene"), c.String("assets"), cf, f, log.New(os.Stderr, "", 0))
	stdin.Close()
	cmdErr := cmd.Wait()
